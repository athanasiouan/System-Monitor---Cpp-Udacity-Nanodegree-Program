...
long LinuxParser::Jiffies() { 
	string line = "";
  	string cpu = "", sUser = "", sNice = "", sSystem = "", sIdle = "", sIOwait = "", sIRQ = "", sSoftIRQ = "", sSteal = "", sGuest = "", sGuestNice = "";
  	...
  	return (std::stol(sUser) + std::stol(sNice) + std::stol(sSystem) + std::stol(sIRQ) + std::stol(sSoftIRQ) + std::stol(sSteal) + std::stol(sGuest) + std::stol(sGuestNice) + std::stol(sIdle) + std::stol(sIOwait));
}
...
vector<string> LinuxParser::CpuUtilization() { 
	long lOldActiveJiffies = ActiveJiffies();
  	...
  	string shlp = Format::ElapsedTime(((lActiveJiffies-lOldActiveJiffies) - (lIdleJiffies - lOldIdleJiffies))/(lActiveJiffies-lOldActiveJiffies));
	...
	string line;
  	...
  	if(filestream.is_open()){
     	while(std::getline(filestream, line)){
          	...
          	linestream >> key >> value;
          	if(key == "VmData:"){
              	int tmp = (100 * std::stol(value)/1024+0.5);
